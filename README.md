...
just replace `python3` with `python3 rerun.py`, so if you would normally do
```bash
python3 your_script.py arg1 arg2
```
then do this:
```bash
python3 rerun.py your_script.py arg1 arg2
```
...
If `your_script.py` terminates, `rerun` will wait for a change to be made and then will run it again.


It's probably best to turn it into an alias using an absolute path so that you can use it anywhere like this:
```bash
rerun your_script.py arg1 arg2
```

So to your rc file add something like:
```bash
alias rerun='python3 ~/code/py-auto-rerun/rerun.py'
```
